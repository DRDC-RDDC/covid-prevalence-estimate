...
def savecuminf(this_model, trace, pop, rootpath="/content", debug=False):  # debug=True
    ...
    data_now = dict(
        ...
        analysisTime=analysis_date,
        dates=x[todayix],
        cuminf_025=[Ec_t_025[todayix] if Ec_t_025[todayix] >= 0 else 0.0],
        cuminf_50=[Ec_t_50[todayix] if Ec_t_50[todayix] >= 0 else 0.0],
        cuminf_975=[Ec_t_975[todayix] if Ec_t_975[todayix] >= 0 else 0.0],
    )
